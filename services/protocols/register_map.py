...
def render_markdown(
    register_map: RegisterMap, io: IOImage, title: str, status_notes: str = "",
    source: str = "`services/protocols/line_map.py`",
) -> str:
    ...
    `source` names where the map came from (a map file, Phase 9 step 4)."""
    ref = lambda table, a: f"{MODBUS_REF[table] + a:05d}" if table != COIL else f"{a + 1:05d}"  # noqa: E731
    r = register_map.controller_ranges()
    lines = [
        f"# {title}",
        "",
        f"*Generated by `scripts/register_map.py` from {source} -- do not edit by hand.*",
        "",
        ...
        + ("analog outputs, controller status, HMI ack word |" if register_map.status_registers
           else "analog outputs, HMI ack word (no status block) |"),
        "",
    ]
    sections = [
