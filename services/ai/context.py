...
VALUE_SHAPES: dict[str, str] = {
    ...
    "source_bin": '"A" | "B" | "C" (HMI setpoint: the bin the next start draws from; a running line keeps its bin)',
    "recipe": '{A: kg, B: kg, C: kg} (batch recipe setpoints; a bin left out is 0; applies at the next batch start)',
    "hold_s": "number of seconds (batch hold / PROCESSING time setpoint)",
    "select_batch": "true (one-shot: select Batch mode; accepted only at rest)",
    "outlet_stuck": "true/false (the hopper outlet gate never reaches its limit switch)",
    "outlet_plugged": "true/false (the hopper outlet is open but nothing drains)",
    "downstream_stopped": "true/false (the downstream process stops taking material: DS-107.READY drops)",
    "outlet_reset": "true (field reset of the hopper outlet gate actuator)",
    "open_outlet": "true (one-shot Manual pushbutton: open the hopper outlet gate)",
    "close_outlet": "true (one-shot Manual pushbutton: close the hopper outlet gate)",
    "bin_b_level_pct": "number 0-100 (set bin B's level directly)",
    "bin_c_level_pct": "number 0-100 (set bin C's level directly)",
    "gate_b_stuck": "true/false (bin B's gate never reaches its limit switch)",
    "gate_c_stuck": "true/false (bin C's gate never reaches its limit switch)",
    "gate_b_reset": "true (field reset of bin B's gate actuator)",
    "gate_c_reset": "true (field reset of bin C's gate actuator)",
    "sensor_noise": '{tag: "<analog input tag>", amplitude: <number>} (uniform noise within +/- amplitude, engineering units; reproducible)',
    "sensor_drift": '{tag: "<analog input tag>", rate_per_s: <number>} (the reading walks away from the truth at this rate; clamped to range)',
    "sensor_slow": '{tag: "<input tag>", seconds: <number>} (the instrument reports what was true this many seconds ago)',
    ...
    # expect (observations)
    "line_state": '"idle" | "starting" | "running" | "stopping" | "faulted" | "estopped" | "manual" | "loading" | "processing" | "discharging" | "cleaning"',
    "mode": '"auto" | "manual" | "batch"',
    "batch_loaded_kg": "number (this or the last batch's weigh-in, kg)",
    "batches_completed": "integer (batches completed since the controller started)",
    "outlet_open": "true/false (the hopper outlet gate at its open limit)",
    "outlet_open_commanded": "true/false (the controller's hopper outlet output)",
    "source_bin": '"A" | "B" | "C" (the bin the next start draws from)',
    "active_bin": '"A" | "B" | "C" | "none" (the bin the line is drawing from now)',
    "gate_b_open": "true/false",
    "gate_c_open": "true/false",
    "gate_b_open_commanded": "true/false (the controller's bin B gate output)",
    "gate_c_open_commanded": "true/false (the controller's bin C gate output)",
    ...
    "downstream_ready": "true/false (DS-107.READY: the downstream consumer can take material)",
    "discharging": "true/false (material actually leaving the hopper for the downstream consumer)",
