...
*Living document and source of truth for the project. Last updated: 2026-09-22. Status: Phases 0–3 complete (simulation core, Control in Auto mode, and declarative commissioning scenarios); Phase 4 in progress — step 1 (alarm core: latch/first-out/acknowledge) done, 207 tests passing — see `docs/ARCHITECTURE.md` for what's actually implemented.*
...
- ✅ *Alarms — core (Phase 4 step 1)*: `Alarm`/`AlarmManager` (`services/control/alarms.py`) — latching, first-out, and acknowledge for the line's 9 alarm conditions (8 trip + bin-low warning), built directly on `Interlocks` and the device-control fault flags. ⬜ *Not yet wired into `LineController`* — the new "no active latched alarms" start permissive and the `acknowledge()` operator command are step 2.
...
### Phase 4 — Fault injection, alarms (in progress)
...
- ⬜ **Step 2 — wire into `LineController`.** The new "no active latched alarms" start permissive (the TODO already sitting in `Interlocks.start_permissives_ok()`'s own docstring), an `acknowledge()` operator command alongside `start()`/`stop()`/`reset()`, and `LineController.scan()` calling `AlarmManager.scan()` each tick. Resolves how `fault_reason` relates to the alarm that caused it.
- ⬜ **Step 3 — surface through Testing.** Extend `vocabulary.py` (`acknowledge` action, `latched_alarms`/`any_unacknowledged_trip` read fields), write the scenario that finally covers §6.3's 8th interlock row, confirm `scripts/scenario_report.py` flips to 8/8.
- ⬜ **Step 4 (optional/separate)** — the two §5.4 fault hooks that don't exist yet: feeder jam (downstream weight-trend detection) and sensor failure (analog out-of-range, discrete stuck). Neither maps to an existing interlock row, so nothing above depends on them; deferred unless full §5.4 parity is wanted in this phase.
