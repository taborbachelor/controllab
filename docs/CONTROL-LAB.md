...
*Living document and source of truth for the project. Last updated: 2026-09-22. Status: Phases 0–4 complete (simulation core, Control in Auto mode, declarative commissioning scenarios — all 8 of §6.3's interlocks covered, 0 gaps — and alarm management: latching, first-out, acknowledge). Phase 4's step 4 (feeder jam + sensor failure fault hooks) deliberately deferred — see §10's Phase 4 entry. Phase 5 in progress — step 1 (generic sampled tag history) done. 220 tests passing — see `docs/ARCHITECTURE.md` for what's actually implemented.*
...
- ⬜ **Step 2 — state/alarm telemetry observer.** A diffing observer over `LineController.state` and `AlarmManager`'s alarm set, called once per tick alongside `TagHistory.record()`. Zero changes to `line_controller.py`/`alarms.py`.
