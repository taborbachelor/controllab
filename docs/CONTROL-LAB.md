...
- ✅ **Step 4 — retarget Phase 1's scenarios through Control.** Not a literal port: `tests/integration/test_line_controller.py` proves the same physical outcomes (conservation, zero spillage, E-stop holds until reset) driven entirely through `LineController`, but "spillage from the wrong start order" has no equivalent — once Control enforces the downstream-first sequence, that failure mode isn't reachable through it at all, which is itself the proof.
