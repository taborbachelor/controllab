...
**I/O**: A tag table shared between Simulation and Control — tag name, type (DI/DO/AI/AO), engineering units, current value — kept deliberately simple, a validated dictionary rather than a framework. Not yet built: it's the first thing Phase 2 (Control) needs, and building it before Control exists to consume it would mean guessing at its shape (see `docs/ARCHITECTURE.md`, Reconciliation notes).
...
### Phase 2 — Control
- The I/O image (tag table) — the first thing this phase needs, deferred out of Phase 1 (see `docs/ARCHITECTURE.md`, Reconciliation notes).
- Motor and gate control modules: run/open commands issued through the I/O image, not directly against simulation objects.
- Line control: the mode manager (Auto first; Manual once Auto is proven — see §6.1), the line state machine (§6.2), start/stop sequences, interlocks (§6.3), hopper hysteresis control.

**Done when:** the start/stop sequences and interlocks from §6 are driven through Control (not by tests calling `Plant`'s devices directly, as Phase 1's tests do), and the same conservation/no-spillage/E-stop scenarios from Phase 1 still pass driven that way.
