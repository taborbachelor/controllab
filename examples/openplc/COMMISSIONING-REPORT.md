...
**Overall: PASS** — 15/15 scenarios passed; 8/8 interlocks covered (0 not yet applicable, 0 gap(s)).
...
| ✅ PASS | Conveyor Fails To Prove Running | 1.30 s | 3.00 s | 1.70 s | `faults/conveyor_fail_to_start.yaml` |
| ✅ PASS | Conveyor Trip While Running Faults The Line | 0.40 s | 0.50 s | 0.10 s | `faults/conveyor_trip_while_running.yaml` |
| ✅ PASS | Feeder Fails To Prove Running | 2.60 s | 5.00 s | 2.40 s | `faults/feeder_fail_to_start.yaml` |
| ✅ PASS | Feeder Trip While Running Faults The Line | 0.30 s | 0.50 s | 0.20 s | `faults/feeder_trip_while_running.yaml` |
| ✅ PASS | Gate Fails To Prove Open | 2.50 s | 5.00 s | 2.50 s | `faults/gate_travel_timeout.yaml` |
...
| ✅ PASS | Conveyor Emergency Stop | 0.40 s | 0.50 s | 0.10 s | `safety/estop_from_running.yaml` |
...
| Conveyor Fails To Prove Running | 3/3 | 1.20 s | 1.30 s | 1.20, 1.20, 1.30 |
| Conveyor Trip While Running Faults The Line | 3/3 | 0.30 s | 0.40 s | 0.30, 0.30, 0.40 |
| Feeder Fails To Prove Running | 3/3 | 2.60 s | 2.60 s | 2.60, 2.60, 2.60 |
| Feeder Trip While Running Faults The Line | 3/3 | 0.30 s | 0.30 s | 0.30, 0.30, 0.30 |
| Gate Fails To Prove Open | 3/3 | 2.50 s | 2.50 s | 2.50, 2.50, 2.50 |
...
| Conveyor Emergency Stop | 3/3 | 0.30 s | 0.40 s | 0.30, 0.40, 0.30 |
...
| ✅ covered | Hopper high-high | Trip | Hopper High-High Faults The Line While Running | — |
| ✅ covered | Any motor fail-to-start / trip | Trip | Conveyor Fails To Prove Running<br>Conveyor Trip While Running Faults The Line<br>Feeder Fails To Prove Running<br>Feeder Trip While Running Faults The Line | — |
...
| 2.20 | response | Line state idle → **starting** |
| 3.20 | response | Line state starting → **faulted** (conveyor failed to prove running) |
| 3.20 | response | Alarm **M-104.START_PROOF** active — Conveyor failed to prove running · **first-out** |
...
| 1.60 | setup | Line state idle → **starting** |
| 3.30 | setup | Line state starting → **running** |
| 4.20 | response | Line state running → **estopped** (e-stop) |
| 4.20 | response | Alarm **ES-001.TRIP** active — E-stop tripped · **first-out** |
